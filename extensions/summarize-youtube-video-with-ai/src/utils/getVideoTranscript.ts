...
export async function getVideoTranscript(
	...
	try {
		...
		if (!transcriptEndpointMatch) {
			transcriptEndpointMatch = videoPageHtml.match(
				/"getTranscriptEndpoint":[^}]*"params":"([^"]+)"/,
			);
		}

		if (!transcriptEndpointMatch) {
			const possibleParams = videoPageHtml.match(
				/"params":"([A-Za-z0-9+/=%]{20,})"/g,
			);
			...
			if (possibleParams && possibleParams.length > 0) {
				for (const paramString of possibleParams) {
					...
					if (paramMatch) {
						...
						const contextBefore = videoPageHtml.substring(
							Math.max(0, paramIndex - 200),
							paramIndex,
						);
						const contextAfter = videoPageHtml.substring(
							paramIndex,
							Math.min(videoPageHtml.length, paramIndex + 200),
						);
		...
		const transcriptData = (await transcriptResponse.json()) as Record<
			string,
			unknown
		>;
		const actions = transcriptData?.actions as
			| Array<Record<string, unknown>>
			| undefined;
		const transcriptBody = actions?.[0]?.updateEngagementPanelAction as
			| Record<string, unknown>
			| undefined;
		const content = transcriptBody?.content as
			| Record<string, unknown>
			| undefined;
		...
		let transcriptBodyRenderer = transcriptContent?.transcriptBodyRenderer as
			| Record<string, unknown>
			| undefined;

		if (!transcriptBodyRenderer) {
			const searchPanelRenderer =
				transcriptContent?.transcriptSearchPanelRenderer as
					| Record<string, unknown>
					| undefined;
			const body = searchPanelRenderer?.body as
				| Record<string, unknown>
				| undefined;
			transcriptBodyRenderer = body?.transcriptSegmentListRenderer as
				| Record<string, unknown>
				| undefined;

			if (!transcriptBodyRenderer) {
				transcriptBodyRenderer = searchPanelRenderer?.footer as
					| Record<string, unknown>
					| undefined;
			}
		}
		...
		if (
			...
			!transcriptBodyRenderer?.initialSegments
		) {
			showToast({
				style: Toast.Style.Failure,
				title: "❗",
		...
		if (transcriptBodyRenderer.initialSegments) {
			const initialSegments = transcriptBodyRenderer.initialSegments as Array<
				Record<string, unknown>
			>;

			transcriptText = initialSegments
				.map((segment: Record<string, unknown>) => {
					const transcriptSegmentRenderer = segment.transcriptSegmentRenderer as
						| Record<string, unknown>
						| undefined;
					const snippet = transcriptSegmentRenderer?.snippet as
						| Record<string, unknown>
						| undefined;
					const runs = snippet?.runs as
						| Array<Record<string, unknown>>
						| undefined;
					...
					return (
						runs
							?.map(
								(run: Record<string, unknown>) => (run.text as string) || "",
							)
							.join("") || ""
					);
				})
			...
			const cueGroups = transcriptBodyRenderer.cueGroups as Array<
				Record<string, unknown>
			>;
			transcriptText = cueGroups
				.map((cueGroup: Record<string, unknown>) => {
					const renderer = cueGroup.transcriptCueGroupRenderer as
						| Record<string, unknown>
						| undefined;
					const cues = renderer?.cues as
						| Array<Record<string, unknown>>
						| undefined;
					return (
						cues
							?.map((cue: Record<string, unknown>) => {
								const cueRenderer = cue.transcriptCueRenderer as
									| Record<string, unknown>
									| undefined;
								const cueData = cueRenderer?.cue as
									| Record<string, unknown>
									| undefined;
